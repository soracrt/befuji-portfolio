...
const fieldClass =
  'font-sans text-sm text-ink bg-transparent border-b border-black/20 py-3 outline-none focus:border-black transition-colors duration-200 w-full placeholder:text-black/30'
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className="w-full flex items-center justify-between border-b border-black/20 py-3 bg-transparent outline-none focus:border-black transition-colors duration-200"
        style={{ color: value ? '#0A0A0A' : 'rgba(0,0,0,0.3)' }}
      >
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute top-full left-0 right-0 z-20 bg-[#F9F9F6] shadow-[0_6px_28px_rgba(0,0,0,0.09)]"
          >
            {SERVICES.map((s) => (
              <button
                key={s.value}
